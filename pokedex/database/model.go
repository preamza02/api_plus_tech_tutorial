...
type PokemonModel struct {
	...
	Types         []PokemonTypeModel `bun:"m2m:pokemon_types,join:PokemonID=PokemonID"`
...
type PokemonTypesModel struct {
	...
	PokemonTypeID int64             `bun:"pokemon_type_id,pk"`
	Pokemon       *PokemonModel     `bun:"rel:belongs-to,join:pokemon_id=id"`
	PokemonType   *PokemonTypeModel `bun:"rel:belongs-to,join:pokemon_type_id=id"`
}
